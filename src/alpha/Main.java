...
public class Main extends Application{
  ...
  @Override public void start(Stage stage) {
    ...
    scene.setOnKeyPressed(new EventHandler<KeyEvent>(){
      @Override
        public void handle(KeyEvent event) {
          if (event.getCode()==KeyCode.LEFT) engine.setChildCommand(User.COMMAND.LEFT);
          if (event.getCode()==KeyCode.RIGHT) engine.setChildCommand(User.COMMAND.RIGHT);
          if (event.getCode()==KeyCode.UP) engine.setChildCommand(User.COMMAND.UP);
          if (event.getCode()==KeyCode.DOWN) engine.setChildCommand(User.COMMAND.DOWN);
          event.consume();
        }
    });
    scene.setOnKeyReleased(new EventHandler<KeyEvent>(){
      @Override
        public void handle(KeyEvent event) {
          if (event.getCode()==KeyCode.LEFT) engine.releaseChildCommand(User.COMMAND.LEFT);
          if (event.getCode()==KeyCode.RIGHT) engine.releaseChildCommand(User.COMMAND.RIGHT);
          if (event.getCode()==KeyCode.UP) engine.releaseChildCommand(User.COMMAND.UP);
          if (event.getCode()==KeyCode.DOWN) engine.releaseChildCommand(User.COMMAND.DOWN);
          event.consume();
        }
    });
    ...
    stage.setWidth(HardCodedParameters.defaultWidth + 530);
